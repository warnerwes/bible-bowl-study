...
const { buildGatewayLink, buildReadLink, buildReadLinks, renderPlan } = await import(src("reading-plan.js"));
...
test("reading plan render builds per-chapter reader links and one gateway fallback", () => {
  ...
  const readLinks = buildReadLinks({
    week: 1,
    book: "1 Corinthians",
    chapters: [1, 2],
    reference: "1 Corinthians 1-2",
  ...
  assert.deepEqual(
    readLinks.map((entry) => entry.href),
    [
      "reader.html?ref=1%20Corinthians%201",
      "reader.html?ref=1%20Corinthians%202",
    ]
  );
  ...
  assert.equal(firstLinks.length, 3);
  assert.equal(firstLinks[0].href, "reader.html?ref=1%20Corinthians%201");
  assert.equal(firstLinks[0].textContent, "Read 1 Corinthians 1 here");
  assert.equal(firstLinks[0].target, "");
  assert.equal(firstLinks[1].href, "reader.html?ref=1%20Corinthians%202");
  assert.equal(firstLinks[1].textContent, "Read 1 Corinthians 2 here");
  assert.match(firstLinks[2].href, /^https:\/\/www\.biblegateway\.com\/passage\//);
  assert.equal(firstLinks[2].textContent, "Open on Bible Gateway ↗");
